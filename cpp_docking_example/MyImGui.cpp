...
bool MyImGui::Init(HWND hWnd, const wchar_t* windowTitle, int width, int height, int flag, float FontSize)
{
    ...
    {
        std::wcerr << L"Warning: Invalid font data, using default ImGui font" << std::endl;
        return true; // Continue with default font
    }
    
    // Configure font loading settings
    ImFontConfig Font_cfg;
    Font_cfg.FontDataOwnedByAtlas = false;  // We manage font data lifetime
    
    // Load custom font with Chinese character support
    // Font_data and Font_size are defined in Font.h
    ImFont* Font = io.Fonts->AddFontFromMemoryTTF(
        (void*)Font_data,                    // Font data pointer
        Font_size,                           // Font data size in bytes
        FontSize,                            // Font size in pixels
        &Font_cfg,                           // Font configuration
        io.Fonts->GetGlyphRangesChineseFull() // Include full Chinese character set
    );
    
    // Validate font loading
    if (!Font)
    {
        std::wcerr << L"Warning: Failed to load custom font, using default ImGui font" << std::endl;
        // Continue execution with default font - not a critical error
    }
    else
    {
        std::wcout << L"Successfully loaded custom font with Chinese support" << std::endl;
    }
